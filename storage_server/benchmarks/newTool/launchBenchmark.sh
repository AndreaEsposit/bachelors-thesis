...
pdsh -w $USERNAME@bbchain[$FB-$LB] "cd Practice/&&git reset --hard; git clean -f -d; git checkout HEAD; git pull"

# Task 2:
#   launhes the benchmark on all the bbchain machines
pdsh -w $USERNAME@bbchain[$FB-$LB] "cd Practice/storage_server/benchmarks/multi/&&chmod +x benchmark.sh&&./benchmark.sh $CLIENTS $REQUESTS $MODE $TYPE"
